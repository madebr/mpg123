...
def find_objects(tree, root):
    result = []
    assembly_names = [str(s.stem) for s in (root / "src/libmpg123").iterdir() if s.suffix == ".S"]
    for root, _, files in os.walk(tree):
        for f in files:
            file_name = f.removesuffix(".S.obj").removesuffix(".S.o").removesuffix(".o").removesuffix(".obj")
            if file_name in assembly_names:
...
def configure_build(root: pathlib.Path, dirid: str, fast: bool, cmake_flags: list[str]):
    ...
    try:
        original_objects = find_objects(build_dir, root=root)
...
def build_unix(root: pathlib.Path, fast: bool):
    ...
    configure_build(root=root, fast=fast, dirid="original", cmake_flags=["-DCMAKE_BUILD_TYPE=Release"])
    configure_build(root=root, fast=fast, dirid="original32", cmake_flags=["-DCMAKE_BUILD_TYPE=Release", "-DCMAKE_C_FLAGS=-m32", "-DCMAKE_ASM_FLAGS=-m32"])

    subprocess.check_call(["git", "-C", str(root), "checkout", GIT_INTEL])
    configure_build(root=root, fast=fast, dirid="intel", cmake_flags=["-DCMAKE_BUILD_TYPE=Release"])
    configure_build(root=root, fast=fast, dirid="intel32", cmake_flags=["-DCMAKE_BUILD_TYPE=Release", "-DCMAKE_C_FLAGS=-m32", "-DCMAKE_ASM_FLAGS=-m32"])
...
def build_msvc(root: pathlib.Path, fast: bool, generator: str):
    ...
    subprocess.check_call(["git", "-C", str(root), "checkout", GIT_INTEL])
    configure_build(root=root, fast=fast, dirid="intel", cmake_flags=["-G", generator, "-A", "x64"])
    configure_build(root=root, fast=fast, dirid="intel32", cmake_flags=["-G", generator, "-A", "Win32"])
...
def disassemble(dirid: str, root: pathlib.Path, compare_dir: pathlib.Path, disassemble_callback: typing.Callback[[pathlib.Path], str]):
    object_paths = find_objects(build_directory_for_id(root=root, dirid=dirid), root=root)
    d = compare_dir / dirid
...
def compare_dirids(disassemble_callback: typing.Callback[[pathlib.Path], str], compare_dir: pathlib.Path, root: pathlib.Path, dirid_original: str, dirid_intel: str):
    ...
    for original_path in original_paths:
        ...
        for intel_disasm_suffix in (".S.txt", ".txt"):
